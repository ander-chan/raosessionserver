...
User.getByLogin = function (login,callback) {
    var u = {id: 0};
    pool.query('SELECT * FROM users where user=\'' + login + '\';', (err, rows, fields) => {
        if (err) {
            console.error('getUserLogin ', err);
            return;
        }
...
User.getUserIndexById = function (id) {
    return _.findIndex(this.onlineUsers, (u) => {
        return u.id == id
    });
};
...
User.getUserById = function (id) {
    let index = _.findIndex(this.onlineUsers, (u) => {
        return u.id == id
    });
...
User.getUserIndexByName = function (name) {
    return _.findIndex(this.onlineUsers, (u) => {
        return u.user.toLowerCase() === name.toLowerCase()
    });
}
